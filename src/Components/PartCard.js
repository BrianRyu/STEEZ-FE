    ...
    render(){
        return(
            <div className="part-grid-item">
                <span onClick={this.handleClick} className="part-add" role="img" aria-label="x">➕</span>
                <p className="product-font">{this.props.part.name}</p>
                <br />
                <img className="product-part-image" src={this.props.part.img_url} alt={this.props.part.name} />
                <p id="part-description" className="product-font">{this.props.part.description}</p>
            </div>
        )
    }
}
...
export default connect(stateToProps, {addPiece})(withRouter(PartCard));